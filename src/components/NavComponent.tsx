    ...
    <nav class={styles.nav}>
      <div class={styles['profile-card']}>
        <div class={styles['nav-app-controls']}>
          ...
          <button class={`${appStyles.button} ${appStyles.link} ${styles['new-note']}`} onClick={onNewNoteClicked}>
            <Fa class={styles['nav-icon']} icon={faPlusSquare} />
          </button>
        </div>
        <div class={styles['nav-auth-controls']}>
          <a class={`${appStyles.button} ${appStyles.link} ${styles['logout']}`} href={`${API_HOST}/logout`}>
